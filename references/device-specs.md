...
- The claim that both displays share the same aspect ratio is secondary-source-only, and **the file's own numbers don't quite support it**: 1398×2034 → 0.687; 1878×2670 → 0.703 — close, but about 2% apart, not identical. Treat "same aspect ratio" as an approximation at best, not a confirmed fact.
- **App Store Connect's screenshot-canvas dimensions differ from the native panel pixel counts above**: the *inner-display screenshot* requirement is 2007 × 2853 px (portrait) — not 1878×2670. One observation worth flagging rather than asserting as fact: the **outer** screenshot size (1398×2034) matches the outer panel resolution exactly, while the **inner** screenshot size is the inner panel resolution scaled by a near-uniform ×1.069 (2007/1878 ≈ 1.0687; 2853/2670 ≈ 1.0685) — close to the 460/430 ppi ratio between the two displays (≈1.070). That pattern suggests the inner display's screenshot canvas may render at a higher effective pixel density than its native panel and downsample, but this is this skill's own inference from the numbers, not something Apple states — don't repeat it as confirmed. Don't conflate "native panel resolution" (this file) with "App Store Connect screenshot canvas size" (see [app-store-submission.md](app-store-submission.md)) — they are two different numbers for two different purposes regardless of why they differ.
