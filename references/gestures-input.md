...
**Limitations**
- Custom gestures must not conflict with gestures that access system UI (e.g., edge-swipe reserved for system back/Control Center/Notification Center). On iPhone Duo, the same discipline extends to the split-view-multitasking Home Gesture and any system-reserved fold-adjacent zones — see [iphone-duo.md](iphone-duo.md).
...
**Limitations**
- Minimum hit target: **44×44pt** — this is a hard usability/accessibility floor, not a style preference (see [accessibility.md](accessibility.md)).
...
**Design implications**
- On iPhone Duo, text fields follow the same virtual-keyboard/secure-field rules regardless of which display is active — see the Virtual Keyboards section above; no Duo-specific text-field behavior beyond the general vertical-bar/safe-area handling already covered in [iphone-duo.md](iphone-duo.md).
