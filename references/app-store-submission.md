...
- The inner-display screenshot canvas (2007×2853px) does not match the native inner-panel resolution (1878×2670px, per [device-specs.md](device-specs.md)) — not necessarily an error (the outer screenshot size *does* match its panel exactly, while the inner one is scaled by a factor close to the two displays' ppi ratio; see device-specs.md for that observation), but don't conflate the two numbers when producing assets.
