...
1. **[references/iphone-duo.md](references/iphone-duo.md)** — the primary HIG reference. Device anatomy (outer/inner display + cameras), device poses, reserved regions, the **displacement** design pattern, dynamic layouts, split views, arrangement views, vertical toolbar/tab-bar placement, sheets, system multitasking/Picture-in-Picture, and games — each split into Capabilities / Limitations / Design implications, cited to the source HIG page and Apple's own Sept 2026 Tech Talks.
...
- There are **two different, unreconciled statements about `UIRequiresFullScreen`** from two Apple sources — presented side by side in xcode-development.md rather than resolved, since resolving it wrongly would be worse than leaving it open.
